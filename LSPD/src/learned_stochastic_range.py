...
limited_angle = 60  # max angle range
n_angles = 60   # number of projection angles
max_radian = limited_angle*(np.pi/180)
operator_n_angles = 180/(limited_angle/n_angles)
print("operator number angles:", operator_n_angles)
...
geometry = odl.tomo.parallel_beam_geometry(space, num_angles=operator_n_angles)
...
with tf.name_scope('placeholders'):
    ...
    #epoch_order_selection = tf.placeholder(tf.int32, shape=[n_batches, angles_in_batch], name='order_selection')
    #selection = tf.placeholder(tf.float32, shape=[angles_in_batch], name='iteration_selection')
...
with tf.name_scope('tomography'):
    with tf.name_scope('initial_values'): #using this convention only for tensorboard
        ...
        dual = tf.placeholder(tf.float32, shape=[None, angles_in_batch, 183, 5], name='dual')
        #primal = tf.placeholder(tf.float32, shape=[None, size, size, 0], name='primal')
    ...
    for i in range(n_batches):  # iterations, the amount of projection batches we have
        ...
        with tf.variable_scope('primal_iterate_{}'.format(i)):
            ...
            # evalop = odl_op_partial_layer_adjoint(dual[..., 0:1], )
